...
func _ready():
		# Get the joystick with the correct name
	joystick = get_node_or_null("/root/Main/Control/TouchControls/Virtual Joystick")
	
	if joystick:
		print("Joystick found successfully!")
	else:
		print("Joystick still not found")
...
func _input(event):
	...
	if event is InputEventKey and event.pressed:
		...
		# Only handle screen drags that are NOT in the joystick area
	if event is InputEventScreenDrag:
		if not joystick: #safety check
			return
		
		# Only process camera rotation if drag is NOT in joystick area
		if not joystick._is_point_inside_joystick_area(event.position):
			set_rotation_target(event.relative)
			get_viewport().set_input_as_handled()
	
	# Regular mouse motion with captured mouse
